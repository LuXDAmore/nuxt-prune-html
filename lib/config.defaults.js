...
export default {
    ...
    matchUserAgent: null, // Remove selectors if this userAgent is matched, could be a string or an array of strings
    headerName: 'user-agent', // The header-key value used for the detection from the request
    auditUserAgent: 'lighthouse', // Value of the Audit User Agent request header to check, could be also an array of strings
    // TODO: queryParamToExclude? checkGenerateQueryParams? pruneWithQueryParamOnly? checkHeaderExistenceOnly? onBeforePrune? onAfterPrune? selectorToKeep(array)?
    // TODO: fix nuxt.config
    checkRouteQueryParams: false, // Include or Exclude a page if a specified query params is matched in the url, during the routing
    checkGenerateQueryParams: false, // Include or Exclude a page if a specified query params is matched in the url, during the generate-process
    pruneWithQueryParamOnly: false, // Force to prune only if a query param `key-value` is matched and not by the user-agent
    queryParamToPrune: {
        // trigger the pruning if `checkRouteQueryParams` is `true` and the url is like `/?prune=1`
        key: 'prune',
        value: '1',
    },
    queryParamToExclude: {
        // exclude the pruning if `checkRouteQueryParams` is `true` and the url is like `/?prune=0`
        key: 'prune',
        value: '0',
    },
};
